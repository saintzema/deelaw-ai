...
const DashboardLayout: React.FC<DashboardLayoutProps> = ({ children }) => {
  ...
  return (
    <div className={`min-h-screen ${theme === 'dark' ? 'bg-bolt-darker' : 'bg-gray-50'}`}>
      {/* Top Bar */}
      <header className={`fixed top-0 left-0 right-0 h-16 ${
        theme === 'dark' 
          ? 'bg-bolt-darker border-bolt-gray-800' 
        ...
        <div className="h-full px-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <button
              ...
              className={`p-2 rounded-lg transition-colors ${
                theme === 'dark'
                  ? 'hover:bg-bolt-gray-800/50 text-bolt-gray-300'
            ...
            <div className={`text-right mr-4 ${
              ...
              <p className={`text-sm ${
                theme === 'dark' ? 'text-bolt-gray-400' : 'text-gray-500'
              }`}>
                {user?.tokens.words.toLocaleString()} words remaining
              </p>
            </div>
      ...
      {/* Sidebar */}
      <aside className={`fixed top-16 bottom-0 left-0 w-64 ${
        theme === 'dark'
          ? 'bg-bolt-darker border-bolt-gray-800'
          : 'bg-white border-gray-200'
      } border-r transform transition-transform duration-300 z-30 ${
